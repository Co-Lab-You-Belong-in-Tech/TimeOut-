...
const Month = () => {
  ...
  const retrievePreviousWeekData = async () => {
    try {
      const userId = await AsyncStorage.getItem('userId');
      const response = await fetch(`https://timeout-api.onrender.com/api/stats/${userId}/2023-12-31`);
      const responseData = await response.json();
      // Extract the required data for the chart
    
      const previousWeekStats = responseData.allMonthsStats.monthlyStats;
  ...
  const fetchDataFromAPI = async () => {
    try {
      ...
      const currentWeekStats = responseData.allMonthsStats.monthlyStats;
  ...
  const totalTime = async () => {
    try {
      ...
      const response = await fetch(`https://timeout-api.onrender.com/api/stats/${userId}`);
      const responseData = await response.json();
      console.log(responseData.allMonthsStats.monthlyStats)
      
      
      // Extract the total weekly time in seconds
       
        // Extract the total monthly time data
        const monthlyStats = responseData.allMonthsStats.monthlyStats;
        ...
        if (currentMonthData) {
            // Extract the total time for the current month in minutes
            const totalMinutes = currentMonthData.totalTimeLog;
            ...
            formattedTotalTime += `${minutes}min`;

            // Update the state with the formatted total time
            setTotalWeekTime(formattedTotalTime);
        } else {
            // If no data found for the current month, set total time to 0
            setTotalWeekTime('0min');
        }
    } catch (error) {
        console.error('Error fetching total monthly time:', error);
    }
};

// Function to get the current month (returns a string like "Jan", "Feb", etc.)
